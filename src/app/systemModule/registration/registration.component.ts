...
export class RegistrationComponent extends BaseComponent implements OnInit  {
    ...
    getLocation(address: string){
        ...
        this.service.GetAddress(address).subscribe((res) => {
            for (const adr of res.suggestions) this.Addresses.push(adr.value);
        });
    ...
    checkWorkingTime(){
        ...
        for (const i of this.Coworking.working_days){
            const beginArr = i.begin_work.split(':');
            const endArr = i.end_work.split(':');
            ...
            this.Days[index].finish_work < this.Days[index].start_work)
        {
            ...
            if (this.Days[index].start_work.indexOf('_') === -1){
                if (parseInt(this.Days[index].start_work[0] + this.Days[index].start_work[1]) <= 21 && parseInt(this.Days[index].start_work[3] + this.Days[index].start_work[4]) <= 59){
                    const beginArr = this.Days[index].start_work.split(':');
                    let endHour;
                    if (+beginArr[0] <= 7){
                        const for_parse = parseInt(beginArr[0][1]);
              ...
              autocomplete.addListener('place_changed', () => {
               ...
               if (place.geometry === undefined || place.geometry === null ){
                ...
                return;
                   ...
                   this.Coworking.address =  autocomplete.getPlace().formatted_address;
